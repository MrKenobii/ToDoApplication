...
function showTasks(){
  ...
  const pendingTasksNumber = document.querySelector(".pending-tasks");
  pendingTasksNumber.textContent = listArray.length; //passing the array length in pendingtask
  if(listArray.length > 0){ //if array length is greater than 0
    deleteAllButton.classList.add("active"); //active the delete button
  }else{
    deleteAllButton.classList.remove("active"); //unactive the delete button
  }
  let newLiTag = "";
